...
std::vector<LPCSTR> staticMorh = { "Форма", "Окраска кожуры", "Окраска мякоти", "Глубина глазков", "Глубина столонного следа", "Поверхность кожуры клубня" };

std::vector<LPCSTR> staticCul = { "Разваримость клубней", "Консистенция мякоти", "Рассыпчатость", "Водянистость клубней", "Запах варёного картофеля", "Вкус варёного картофеля", "Потемнение мякоти сырого картофеля", "Потемнение мякоти варёного картофеля" };

std::vector<LPCSTR> staticStand = { "Полевая устойчивость к фитофторозу, балл", "Способность к ягодообразованию от самоопыления (балл)", "Продолжительность цветения,дни", "Группа спелости", "Урожайность, %" };

std::vector<LPCSTR> compliteBTN = { "Добавить" };

std::vector<LPCSTR> editName = { "" };

std::vector<HMENU> compliteAddBTNC = { (HMENU)AddClickButton };
